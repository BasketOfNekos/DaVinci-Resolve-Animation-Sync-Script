...
You'll be able to find it in the fusion menu under the Fuses tab. Look for "Wav_Interpolator" (right click > Add Tool > Fuses > Wav_Interpolator)
...
The math that happens when you slow down/speed up an animation may ask for frame content that exists between 2 others. Davinci has algorithems for calculating this with video inputs (Nearest, blend, flow) which will use your project's default settings or your custom settings to fill it in. To use a non-default mode put a 'TimeSpeed' node between this one and your video, and set your desired mode in the node's setting. To make optical flow work you'll need an 'OpticalFlow' node before your 'TimeSpeed' node.
